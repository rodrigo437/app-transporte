...
# BLOQUE DE RESUMEN CORREGIDO
for i, solicitud in enumerate(lista_solicitudes):
    estado = solicitud["Estado"]
    color_estado = "green" if estado == "Pendiente de asignación" else "gray"
    titulo = f"<span style='color:{color_estado}; font-weight:bold;'>[{estado}]</span> Solicitud #{i+1} - {solicitud['Destino']} ({solicitud['Tipo de camión']})"
    with st.expander(titulo, expanded=False):
        st.write(solicitud)
        st.markdown("**Historial de estados:**")
        for est in ["Pendiente de asignación", "Solicitud asignada", "Cargando solicitud", "Trasladando solicitud", "Entrega de solicitud"]:
            if est == solicitud["Estado"]:
                st.markdown(f"<span style='color:green;font-weight:bold;'>→ {est}</span>", unsafe_allow_html=True)
            else:
                st.markdown(f"<span style='color:gray;'>• {est}</span>", unsafe_allow_html=True)
